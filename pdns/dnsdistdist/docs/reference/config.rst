...
.. function:: addDOHLocal(address, [certFile(s) [, keyFile(s) [, urls [, options]]]])
  ...
  * ``tlsGroups``: str - The TLS supported groups to advertise when the OpenSSL provider is used, in OpenSSL format. For example, ``X25519MLKEM768:X25519:P-256``. Leave empty to use the library default.
  ...
  * ``echKeyFiles``: list - List of files containing ECH configurations and private keys in OpenSSL PEM format, used to enable inbound Encrypted Client Hello. Requires the OpenSSL provider and OpenSSL server-side ECH support. The corresponding public ECH configuration needs to be published separately, for example via HTTPS or SVCB records.
...
.. function:: addTLSLocal(address, certFile(s), keyFile(s) [, options])
  ...
  * ``tlsGroups``: str - The TLS supported groups to advertise when the OpenSSL provider is used, in OpenSSL format. For example, ``X25519MLKEM768:X25519:P-256``. Leave empty to use the library default. With the GnuTLS provider, configure groups via the ``ciphers`` priority string instead.
...
.. function:: newServer(server_string)
  ...
  .. csv-table::
    ...
    ``tlsGroups``                            ``string``            "The TLS supported groups to advertise when the OpenSSL provider is used, in OpenSSL format. For example, ``X25519MLKEM768:X25519:P-256``. Leave empty to use the library default."
